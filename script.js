...
function aktualisiereLatestSichtbarkeit() {
    ...
    allePosterTitle.textContent   = verstecken ? 'Suchergebnisse' : 'Alle Poster';
}
    ...
    modalBody.innerHTML = `
        <div class="modal-poster-wrap">
            ...
            <div class="modal-poster-overlay">
                ...
                ${poster.release_year ? `<span class="modal-year-badge">${poster.release_year}</span>` : ''}
            </div>
        </div>
        <div class="modal-body-content">
            <div class="modal-genre-row">
                <span class="modal-genre-label">Genre</span>
                <span class="modal-genre-tags">${genreText !== 'Nicht angegeben'
                    ? genreText.split(', ').map(g => `<span class="genre-tag">${g}</span>`).join('')
                    : '<span class="genre-tag-empty">Nicht angegeben</span>'
                }</span>
            </div>
...
document.getElementById('contactClose').onclick = () => contactModal.style.display = 'none';
